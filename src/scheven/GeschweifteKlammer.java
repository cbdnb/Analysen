 ...
 *
 */
public class GeschweifteKlammer {
	...
	public static void main(final String[] args) throws IOException {
		...
		printStream = new PrintStream("D:/Analysen/scheven/Geschweifte.txt");
		reader.setStreamFilter(new StringContains("{"));
		reader.forEach(record ->
		{
			...
			for (final Line line : lines14XX) {
				boolean found = false;
				for (final Subfield subfield : line.getSubfields()) {
					if (subfield.getContent().contains("{"))
						found = true;
				}
